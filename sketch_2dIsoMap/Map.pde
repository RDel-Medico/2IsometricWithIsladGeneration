class Map {
  ...
  void generatePerlinNoise() {
    ...
    for (int x = 0; x < w; x++) {
      ...
      for (int y = 0; y < h; y++) {
        ...
        map[x][y] = map(map[x][y] + perinNoise(xoff, yoff), 0, 2, 0, 1 );
      }
      yoff+=0.05;
    }
  }
  ...
  void generateIslandGradient () {
    ...
    for (int x = 0; x < w; x++) {
      for (int y = 0; y < h; y++) {
        ...
        map[x][y] -= map(value, 0, maxValue, 0, 0.7);
      }
    }
  }
  ...
  void newMap() {
    ...
    for (int i = 0; i < w && nbGrassTile < 40; i++) {
      for (int j = 0; j < h && nbGrassTile < 40; j++) {
        if (map[i][j]*255 > 90) {
          nbGrassTile++;
        }
      }
